    ...
    <div class="content-height overflow-y-scroll relative">
      ...
      <div class="flex justify-self-end mb-6">
        <div class="relative group">
          <!-- Sort Button -->
          <div class="dark-button w-fit flex justify-center items-center space-x-1 cursor-pointer">
            <p>Sort</p>
          ...
          <!-- Sort Modal -->
          <div
            class="absolute hidden group-hover:flex flex-col bg-white border border-gray-300 shadow-lg rounded-md py-2 px-4 w-40 right-0 top-full mt-2 z-50">
            <button class="text-sm text-gray-700 hover:text-black hover:bg-gray-100 px-2 py-1 rounded transition">
              Sort by Date
            </button>
            <button class="text-sm text-gray-700 hover:text-black hover:bg-gray-100 px-2 py-1 rounded transition">
              Sort by Title
      ...
      <table class="w-full table-fixed">
        ...
        <tr class="w-full table-auto border-b border-black text-gray-600">
          {{-- Date Input --}}
          <td class="px-2 py-2 text-start border-r border-r-black">{{$expense->date}}</td>

          {{-- Title Input --}}
          <td class="px-2 py-2 text-start border-r border-r-black">{{$expense->title}}</td>

          {{-- Amount Input --}}
          <td class="px-2 py-2 text-start border-r border-r-black">£{{$expense->amount}}</td>

          <td class="flex justify-center items-center h-full py-4">
        ...
        <tr class="w-full table-auto sticky bottom-0 bg-folder">
          <form method="POST" action="/expenses" class="flex space-x-3 relative">
            ...
            <td class="px-2 py-4 text-start border-r border-r-black relative">
              ...
              @error('date')
              <div class="absolute -bottom-10 text-sm rounded-2xl shadow-2xl px-4 py-2 bg-red-600 text-white">
                {{$message}}
              </div>
              @enderror
            </td>
