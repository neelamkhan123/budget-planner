    ...
    <div class="flex flex-col justify-between w-1/2 h-full pt-5">
        <div class="content-height overflow-y-scroll relative">
            <div class="flex justify-between items-center mb-6">
                {{-- Heading --}}
                <h3 class="font-bold text-xl pl-4 text-hotPink tracking-wider flex place-self-center">Decide what you
            ...
            <table class="w-full table-fixed">
                <tr class="border-b border-b-black sticky top-0 bg-folder">
                    ...
                    <th class="w-[30%] pl-2 py-2 text-start border-r font-bold border-r-black">What do you want to buy?
                    </th>
                    <th class="w-[30%] pl-2 py-2 text-start border-r font-bold border-r-black">How much is it?</th>
                    <th class="w-[5%] py-2 border-r border-r-black"><img
                            src="{{ asset('images/approve-accept-icon.svg') }}" alt="checkbox"
                            class="w-5 h-5 mx-auto pb-1"></th>
                    <th class="w-[5%]"></th>
                </tr>

                {{-- Leisure Entries --}}
                @foreach ($leisure as $item)
                <tr class="w-full table-auto border-b border-black text-gray-600">
                    ...
                    <td class="px-2 text-start border-r border-r-black">{{ \Carbon\Carbon::createFromFormat('Y-m-d',
                        $item->date)->format('jS M') }}
                    </td>

                    {{-- Title Input --}}
                    <td class="px-2 text-start border-r border-r-black">{{$item->title}}</td>

                    {{-- Amount Input --}}
                    <td class="px-2 text-start border-r border-r-black">£{{$item->amount}}</td>
                    ...
                    <td class="py-2 px-2 border-l border-l-black">
                        <form method="POST" action="/leisure/{{ $item->id }}" class="flex justify-center items-center">
                            @csrf
                            @method('DELETE')
                ...
                <tr class="w-full table-auto sticky bottom-0 bg-folder">
                    <form method="POST" action="/leisure" class="flex space-x-3 relative">
                        @csrf
                        {{-- Date Input --}}
                        <td class="px-2 py-4 text-start border-r border-r-black relative">
                            <input type="date" name="date" id="date"
                                class="border border-hotPink p-1 rounded-lg text-xs w-full" />

                            @error('date')
                            <div
                                class="absolute -top-6 text-sm rounded-2xl shadow-2xl px-4 py-2 z-400 bg-red-600 text-white">
                                {{$message}}
                            </div>
                            @enderror
                        </td>
                        ...
                        {{-- Amount Input --}}
                        <td class="w-full px-2 py-4 text-start border-r border-r-black flex space-x-1 relative">
                            <span>£</span>
